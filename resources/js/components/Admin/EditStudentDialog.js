...
export default function FormDialog(props) {
  ...
  const [username , setUsername] = React.useState(props.all.username);
  const [password , setPassword] = React.useState(props.all.password);
  const [name , setName] = React.useState(props.all.name);
  const [phoneNumber , setPhoneNumber] = React.useState(props.all.phone_number);
  const [email , setEmail] = React.useState(props.all.email);
  const [nationalCode , setNationalCode] = React.useState(props.all.national_code);
  const [avg , setAvg] = React.useState(props.all.avg);
  ...
  const handleOk = () => {
    ...
    axios({
        method : 'post',
        url : baseUrl+'/api/administrator/student/update/'+props.all.id,
        data : {
  ...
  return (
    <div style={{padding : 20}}>
      <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
        <DialogTitle id="form-dialog-title">ویرایش دانش آموز</DialogTitle>
        <DialogContent>
          <DialogContentText>
             لطفا فرم زیر را پر کنید و دقت داشته باشید که اطلاعات ویرایش شده قابل بازیابی نخواهند بود
