...
export default function FormDialog(props) {
  ...
  const [first_ans , setFirst_ans] = React.useState('');
  const [second_ans , setSecond_ans] = React.useState('');
  const [third_ans , setThird_ans] = React.useState('');
  const [fourth_ans , setFourth_ans] = React.useState('');
  ...
  const handleOk = () => {
    ...
    axios({
        method : 'post',
        url : baseUrl+'/api/administrator/question/create',
        data : formData,
        headers : {
            ...
            if(response.data.status === 's'){
                if(response.data.data.full == true)
                setSuccess(true);
                else{
                  setMessage('برخی آزمون های انتخاب شده به حداکثر رسیده اند');
                  setErrors(true);
                }
  ...
  return (
    <div style={{padding : 20}}>
      ...
      <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
        ...
        <DialogContent>
          <DialogContentText>
              با استفاده از این فرم ابتدا سوالات را تعریف کنید و آنها را به آزمون های مورد نظر اضافه کنید 
          </DialogContentText>
            <Register data={all} />
